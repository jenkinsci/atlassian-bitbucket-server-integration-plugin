...
The Bitbucket integration plugin is the easiest way to connect [Jenkins](http://jenkins.io/) to [Bitbucket Data Center](https://www.atlassian.com/software/bitbucket/enterprise/data-center). With a few simple steps you can configure it to:
...
- Secure credential management in Jenkins for cloning from Bitbucket.
- Adds a Bitbucket Source Code Manager (SCM) to Jenkins, making it easier to create a connection to a Bitbucket repository when setting up a job.
- Automatic webhook creation in a Bitbucket repo when a Jenkins job is saved.
- Quick selection of Bitbucket projects and repos for a Jenkins job through a dropdown.
- The ability to automatically send build statuses to Bitbucket.
- Cloning from Bitbucket [Smart Mirrors](https://confluence.atlassian.com/display/BitbucketServer/smart+mirroring) with no need to modify the clone URL.
...
Note: Bitbucket 6.0 to 7.3 are also supported, but they're not recommended. This is because some plugin features are not available when using these versions. Instead, we recommend using Bitbucket 7.4+. With 7.0+ you can make use of pull request triggers for jobs. With 7.4+ you can set up an Application Link to have access to all plugin features.
...
1. [Add Bitbucket instance details](#add-bitbucket-instance-details).
2. [Create an Application Link](#create-an-application-link). 

### Add Bitbucket instance details

Bitbucket instances are added and configured at the system level. Once they’re added users can select them from the SCM when creating a Jenkins job. You must add at least one Bitbucket instance to Jenkins.

When adding a Bitbucket instance you must add at least one Bitbucket Data Center [HTTP access 
...
3. Enter these instance details:
   ...
   - Instance URL - Enter the Bitbucket base URL. For example, http://localhost:7990/bitbucket. 
...
Creating an Application Link to Jenkins enables additional functionality in Bitbucket. Watch our [video](https://youtu.be/djXwI9OGvhQ) to find out how to do this, or see below for written instructions. This step is only relevant if you’re on Bitbucket 7.4+. 

#### 1. Register Bitbucket as a consumer

There are two parts to creating an Application Link. The first is done in Jenkins and involves registering Bitbucket as a consumer.

<img src="images/createnewconsumer.png" width="600"> <br/>

To register a consumer: 
1. In Jenkins, go to **Manage Jenkins** > **Manage Bitbucket consumers**. 
2. Select **Register new consumer**. 
3. Enter these details:
   - Consumer name: Enter a name to help you identify this Bitbucket instance. For example, Bitbucket Sydney.
...
After you save, you’ll be taken to a page called Application Link details. It’s a good idea to keep this page open when moving onto part 2 so you can copy the details across to Bitbucket. 
...
The second part is done in Bitbucket and involves creating an Application Link to Jenkins. Many of the details you need to do this are on the Application Link details page mentioned in step 1. 

<img src="images/linkapplications.png" width="600"> <br/>

To create the Application Link: 
1. In Bitbucket, go to **Administration** (select the cog in the top navigation bar)  > **Application Links**. 
...
5. Complete the form: 
   ...
   - Shared secret - Enter the secret you created when registering Bitbucket as a consumer. 
...
### Select a Bitbucket instance when creating a Freestyle Job

Once you’ve added a Bitbucket instance to Jenkins, users will be able to select it when creating a job. This will make it easier for them to select the repo to be cloned. They’ll also be able to select the Bitbucket build trigger to automatically create a webhook.

<img src="images/createjob.png" width="600"> <br/>

To select a Bitbucket instance when creating a Freestyle job: 
...
To use a different Jenkinsfile for different branches of your Bitbucket project, you need to create a Multibranch Pipeline and add the Jenkinsfile to the repo of each branch you want to build. Jenkins will then automatically find, manage, and execute these Pipelines.
...
You can then run Bitbucket using [AMPS](https://developer.atlassian.com/server/framework/atlassian-sdk/install-the-atlassian-sdk-on-a-linux-or-mac-system/) with the following command:
```
atlas-run-standalone --product bitbucket --version 6.5.0
```
This will start Bitbucket on [http://localhost:7990/bitbucket](http://localhost:7990/bitbucket).

### Running tests

Unit tests are run with the Surefire plugin using `mvn verify`. They can be skipped using `-DskipTests`.

Integration tests are run under the `it` profile with the Failsafe plugin using `mvn verify -Pit`. The tests will start Bitbucket on [http://localhost:7990/bitbucket](http://localhost:7990/bitbucket) and stop it after they are complete.
...
- Fix JENKINS-69268: When using Shared Libraries that use Bitbucket for the SCM, plugin fails to correctly post build status info
...
### 2.0.0 (9 July 2020)
- Application Links between Jenkins and Bitbucket are now supported
- Build statuses provide test summaries, duration and log links in Bitbucket 7.4 and above
...
- You can now clone from Bitbucket Smart Mirrors without modifying the clone URL

### 1.0-alpha-3 (18 Sep 2019)
- Webhooks are now created in Bitbucket automatically when configuring the SCM
...
- Build status is now posted to Bitbucket after starting and completing a build
